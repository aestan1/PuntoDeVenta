...
namespace Proyecto.Formularios.Ventas
{
    public partial class frmNuevaVenta : Form
    {
        ...
        private void btnCrearVenta_Click(object sender, EventArgs e)
        {
            ...
            if (comboBox1.SelectedIndex==0 && txtpagocon.Text.Trim()=="")
            {
                MessageBox.Show("Para pagos en efectivo, debe ingresar el monto recibido", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            if (comboBox1.SelectedIndex == 0 &&  txtpagocon.Text.Trim() == "0")
            {
                MessageBox.Show("Para pagos en efectivo, el monto recibido no puede ser cero", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }

            if (comboBox1.SelectedIndex ==0 && Convert.ToDouble(txtpagocon.Text)< Convert.ToDouble(txttotalpagar.Text))
            {
                MessageBox.Show("Para pagos en efectivo, el monto recibido no puede ser menor al total", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            Venta oSalida = new Venta()
            {
                ...
                FechaRegistro = DateTime.Now.ToString("yyyy-MM-dd - hh:mm:ss tt", new CultureInfo("en-US")),
                ...
                MedioPago = comboBox1.SelectedItem.ToString()
            ...
            else
            {
                ...
                mdVentaExitosa md = new mdVentaExitosa(oSalida);
        ...
        private void txtdoccliente_keyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyData == Keys.Enter)
            {
                Cliente cl = ClienteLogica.Instancia.Listar(out mensaje).Where(p => p.NumeroDocumento.ToUpper() == txtdoccliente.Text.Trim().ToUpper()).FirstOrDefault();
                if (cl != null)
                {
                    txtdoccliente.BackColor = Color.Honeydew;
                    txtdoccliente.Text = cl.NumeroDocumento;
                    txtnombrecliente.Text = cl.NombreCompleto;                    
